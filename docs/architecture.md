...
- Add-ons (`MACOS_ADDONS`) are profiles too: `active_profiles` lists base, the profile, then the add-ons, and every per-profile file is read in that order. A Mac has one main profile, chosen from `MAIN_PROFILES` at bootstrap; any other profile directory except `base` can be stacked as an add-on, including another main profile (`addon_refusal` in `lib/profile.sh`).
